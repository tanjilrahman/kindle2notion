...
- **🔄 Smart Sync**: Automatically detects existing books and updates them instead of creating duplicates
- **🚫 Duplicate Prevention**: Automatically removes duplicate highlights based on content, location, and page number
- **⚡ High Performance**: Efficient batch uploads (up to 100 blocks per request) for faster syncing
- **🎨 Beautiful Formatting**: Highlights are displayed as quotes with emojis, and notes as callouts for better readability
- **📊 Metadata Tracking**: Automatically tracks highlight counts, last highlighted date, and last sync time
- **🖼️ Book Covers**: Automatically fetches and adds book covers from Google Books API
...
3. Additionally, you may modify some default parameters of the command-line with the following options of the CLI:
   - `--enable_location` Set to False if you don't want to see the "Location" and "Page" information in Notion. (Default: True)
   - `--enable_highlight_date` Set to False if you don't want to see the "Date Added" information in Notion. (Default: True)
   - `--enable_book_cover` Set to False if you don't want to store the book cover in Notion. (Default: True)
   - `--separate_blocks` Set to True to separate each clipping into a separate quote block. Enabling this option significantly decreases upload speed. (Default: False)
...
1. **Duplicate Detection**: The script automatically detects and removes duplicate highlights by comparing the text content (first 50 characters), location, and page number. If a highlight with the same content, location, and page already exists, it will be skipped.

2. **Smart Updates**: When you run the script multiple times:

   - If a book already exists in your Notion database, the script will check if the highlight count has changed
   - If the count differs, it will efficiently update the book by recreating the page with all new highlights
   - If the count matches, it will skip the update (no changes needed)

3. **Efficient Syncing**:

   - Blocks are uploaded in batches of up to 100 for optimal performance
   - The script uses a single Notion connection for all books to minimize overhead
   - Database metadata is cached to avoid repeated API calls

4. **Output Format**:
   - Highlights are displayed as **Quote blocks** with metadata (page, location, date)
   - Notes are displayed as **Callout blocks** with a lightbulb icon
   - Each clipping is separated by a divider for better readability
